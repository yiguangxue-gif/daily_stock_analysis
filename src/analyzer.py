...
"""
===================================
A股自选股智能分析系统 - AI分析层 (A股超神特化·完美加固版)
===================================
...
4. 【终极加强】支持显式思维链(CoT) + 核心指标三剑客(MACD+KDJ+BOLL)
5. 【抗偷懒引擎】强制防 N/A 机制，底层预计算兜底点位。
6. 【极值护盾】底层增加除零(ZeroDivision)与NaN免疫机制，确保一字板、停牌股不崩溃。
"""
...
```json
{
    ...
    "debate_process": {
        "hot_money_trader": "打板接力客发言(查股性/龙虎榜/资金)：...",
        "risk_director": "风控总监发言(查OBV背离/死亡换手/大盘环境)：...",
    ...
    "dashboard": {
        ...
        "battle_plan": {
            "sniper_points": {
                "ideal_buy": "XX元（严禁填N/A，基于MA10或BOLL推算）",
                "secondary_buy": "XX元",
...
## 决策仪表盘最高原则
1. 显式思维链先行：必须在 `debate_process` 中完成游资与风控的实盘博弈。
2. 量化与网格：仓位必须输出明确的百分比(%)。震荡期必须依靠 ATR 输出 T+0 网格间距。
...
### 必答核心检查点：
1. 妖股基因与换手：结合龙虎榜、CCI超卖/超买、连板基因，判定是主流妖股还是边缘跟风盘。注意死亡换手率派发风险！
2. ATR 网格战法：必须根据日均真实波幅，给出明确的 T+0 高抛低吸网格价差（在 `grid_trading_plan` 中）。
3. 量价诱多防线：OBV 资金潮汐与价格是否背离？MACD/KDJ极值发出了什么明确信号？
4. 量化仓位：请务必给出一个百分比仓位数字（如 15% 或 0%）。
5. 止损铁律：如果均线数据缺失，请直接按收盘价下方一定百分比测算防守位，不准留空。
6. 打脸追溯：如果提示了你昨天的判断错误，必须在反思字段中低头认错。
{fallback_hints}
