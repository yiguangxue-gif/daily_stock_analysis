...
"""
===================================
A股游资量化选股雷达 - 弑神满血版 (K线断层拦截 + 脏数据清洗 + AI逻辑终极强化)
===================================

核心重构:
1. 【K线断层拦截护盾】：强制校验 K线复权价 与 Spot真实体现价，偏离超15%直接剔除，彻底根绝除权未同步导致的幻觉测算！
2. 【历史脏数据熔炉】：核算实盘盈亏时，自动过滤 ±60% 以上的夸张断层数据，将其抹平归零，保卫全局 EV 期望值不被污染。
3. 【AI 逻辑终极强化】：强行喂入 ATR 真实波动率并锁死止损止盈位，要求 AI 必须执行 Devil's Advocate (空头致命隐患排查)。
"""
...
class ReboundScreener:
    ...
    def process_review_and_history(self, market_df, lookback_batches=5):
        ...
        try:
            ...
            logger.info(f"🔍 正在执行全局沙盘推演，严格核实实时盈亏 (启动断层脏数据清洗炉)...")
            ...
            for idx, row in df_hist.iterrows():
                ...
                if pd.isna(row.get('Date_T1')) or str(row.get('Date_T1')).strip() == '':
                    ...
                    if code in today_prices and t0_price > 0:
                        ...
                        # 🚀 历史脏数据熔炉：抹平因除权断层导致的大于 60% 的夸张虚假收益
                        if abs(ret_pct) > 60.0:
                            logger.warning(f"🧹 清洗脏数据: {code} 历史记录({t0_price})与现价({curr_p})发生除权断层，强制归零修复！")
                            ret_pct = 0.0
                            curr_p = t0_price # 连同现价一起掩盖，避免报表视觉惊悚
    ...
    def ai_select_top5(self, candidates, macro_news, actual_used_strategy, strategy_reason, review_summary, market_stats, top_sectors, is_market_safe, ai_feedback_data, global_stats, strat_stats):
        ...
        cand_text = ""
        for c in candidates:
            # 🚀 将底层的核心波动率 ATR和精确止损点位 直接喂给 AI，剥夺 AI 的主观瞎猜权！
    ...
    def run_screen(self):
        ...
        for i, (idx, row) in enumerate(candidates.iterrows(), 1):
            ...
            try:
                ...
                # 🚀 绝杀断层Bug：K线最新复权价与Spot现价偏差过大检测
                kline_last_close = df_kline['收盘'].iloc[-1]
                if spot_price > 0 and abs(kline_last_close - spot_price) / spot_price > 0.15:
                    logger.warning(f"🚧 断层护盾拦截: {name}({code}) K线复权价({kline_last_close:.2f})与真实体现价({spot_price:.2f})偏离超15%，除权数据未同步，强制剔除！")
                    continue
